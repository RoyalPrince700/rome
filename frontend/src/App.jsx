...
function App() {
  ...
  const fetchUserAddToCart = useCallback(async () => {
    ...
    if (!SummaryApi.isBackendConfigured) {
      setCartProductCount(localCount);
      return;
    }
    try {
      ...
      const dataApi = await dataResponse.json();
      setCartProductCount((dataApi?.data?.count || 0) + localCount);
    } catch (error) {
  ...
  useEffect(() => {
    const syncLocalCartCount = () => fetchUserAddToCart();
    window.addEventListener('romeLocalCartChange', syncLocalCartCount);
    return () => window.removeEventListener('romeLocalCartChange', syncLocalCartCount);
  }, [fetchUserAddToCart]);
